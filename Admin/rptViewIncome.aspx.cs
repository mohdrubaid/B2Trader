...
public partial class Admin_Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            string incometype = Request.QueryString["type"].ToString();
            loadaccount(username, incometype);
        }
    }
    //for account
    public void loadaccount(string username,string IncomeType)
    {
        try
        {
             string   sql = "select username,credit,debit,date,remark from account where username='" + username + "' and valu='"+IncomeType+"' order by date asc";       
