...
public partial class Admin_WREquest : System.Web.UI.Page
{
    ...
    public void loadlist()
    {
        try
        {
            string sql = "select r.*,rr.name,rr.mobile from TblRWithdraw  r  inner join register rr on r.username=rr.username  where r.status='Pending' order by r.DOR desc";
    ...
    protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        if (e.CommandName == "delete")
        {
            ...
            int a = objamd.WithdrawRequest(Convert.ToInt32(id), "", 0, "", lbpacktype.Text,"", "M");
            warning.Visible = false;
            sccess.Visible = false;
            sccess.Visible = false;
            info.Visible = false;
            danger.Visible = true;
            lbdanger.Text = "Request Rejected";
            loadlist();
        }
        ...
        if (e.CommandName == "Click")
        {
            ...
            int a = objamd.WithdrawRequest(Convert.ToInt32(id), username.Text, amt, Transaction, lbpacktype.Text, "","A");
                ...
                objmail.WithdrawApproved(username.Text, amt.ToString(), lbwallet.Text, email);
                warning.Visible = false;
                ...
                else if (a == -1)
                {
                    ...
                    info.Visible = false;
                    info.Visible = true;
                    lbinfo.Text = "Request Not Approved Successfully ";
                }
            }
            else
            {
