...
public partial class Admin_withdrawApprovedlist : System.Web.UI.Page
{
    ...
    {
        try {
            string sql = "select r.Wallet,r.tds,LEFT(r.PackType, 4) AS CoinType,r.Packtype,r.Rid,r.incomeType, r.username,r.Amount,r.DOR,r.DOA,r.remark,r.status,b.*,r.Payout,r.AdminCharge from TblRWithdraw  r Left Join register b on r.Username=b.Username where r.status='Reject'";
            if (username != "")
            {
