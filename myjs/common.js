...
var params={delay:7,cookie:{expires:2592e3,},message:{'regLevelEvent':[l('ws-regLevel_0'),l('ws-regLevel_1'),l('ws-regLevel_2'),l('ws-regLevel_3'),],'newUserPlaceEvent':l('ws-newUserPlace'),'upgrageEvent':l('ws-upgrade'),'reinvestEvent':l('ws-reinvest'),'missedEthReceive':l('ws-missedEthReceive'),'sentExtraEthDividends':l('ws-sentExtraEthDividends'),'cannotSendMoneyEvent':l('ws-cannotSendMoneyEvent'),'leadingPartner':l('ws-leadingPartner'),'leadingPartnerToUpline':l('ws-leadingPartnerToUpline'),'leadingPlacePurchase':l('ws-leadingPlacePurchase'),},rates:null,levels:null,events:null,messageIteration:0,storage:{},container:{el:$('.alert-socket__items'),cell:$('.alert-socket__cell'),btn:$('.alert-socket__btn'),wrapper:$('.alert-socket__content'),countItems:50,},eventsTypeIcon:{'regLevelEvent':'useradd.svg','newUserPlaceEvent':'newuserplace.svg','upgrageEvent':'upgrade.svg','reinvestEvent':'reinvest.svg','missedEthReceive':'missed.svg','sentExtraEthDividends':'dividends.svg','cannotSendMoneyEvent':'','leadingPartner':'leading.svg','leadingPartnerToUpline':'missed.svg','leadingPlacePurchase':'dividends.svg',}};params.delay*=1000;var socket=io.connect('wss://forsage.io:2087',{rejectUnauthorized:false,rememberTransport:false,secure:true,});socket.on('addEvents',function(data){data=JSON.parse(data);if(!data.events){notify('empty variable "data.events"');return;}
...
function setAssets(assets){let b=$('body');let h=$('head');for(let type in assets){for(i in assets[type]){switch(type){case 'css':if(Array.isArray(assets[type][i])){h.prepend('<link rel="stylesheet" href="'+assets[type][i][0]+'">');}else{h.append('<link rel="stylesheet" href="'+assets[type][i]+'">');}
break;case 'js':if(Array.isArray(assets[type][i])){b.prepend('<script src="'+assets[type][i][0]+'"></script>');}else{b.append('<script src="'+assets[type][i]+'"></script>');}
break;}}}}
...
storage[key]=val;setCookie('storage',JSON.stringify(storage),{expires:31104000});return val;}
...
function setCookie(name,value,options={}){let defaultOptions={path:'/',};options=Object.assign(defaultOptions,options);if(options.expires instanceof Date){options.expires=options.expires.toUTCString();}else{if(options.expires){options.expires=new Date(Date.now()+Number.parseInt(options.expires)*1000);}}
let updatedCookie=encodeURIComponent(name)+"="+encodeURIComponent(value);for(let optionKey in options){updatedCookie+="; "+optionKey;let optionValue=options[optionKey];if(optionValue!==true){updatedCookie+="="+optionValue;}}
document.cookie=updatedCookie;}
(function(){'use strict';$('[data-title]').on('click',function(){})
$('[data-trigger_value]').click(function(){let el,sib,out,val,res;el=out=$(this);if(el.find('span').length){out=el.find('span');}
...
sib.text(res);}});$('[data-spoiler]').click(function(){let key=$(this).data('spoiler');$(key).slideToggle();});window.l=function l(key){if(!config.lang||!config.lang.hasOwnProperty(key)){return 'no translate';}
...
s.remove();};window.Notice=new Vue({el:'#Notice',template:`
        ...
        `,data:{items:[]},mounted(){let s=document.createElement('style');s.innerHTML=`
                .notice {}
                    .notice__darken { position:fixed; top:0; left:0; z-index:100000; width:100%; height:100%; background:rgba(0,0,0,0.5); }
                    .notice__wrap { position:fixed; z-index:100000; background:#fff; color:#000; box-shadow:0 4px 12px rgba(0,0,0,.1); overflow:hidden; border-radius:3px; }
            ...
            `;document.head.appendChild(s);},computed:{item(){return this.items[this.items.length-1];}},methods:{show(text,params){let item=Object.assign({x:'center',y:'top',width:320,type:'',icon:'',text:text,title:'',buttons:{},lifetime:5000,canclose:true,darken:false},params||{});this.items.push(item);if(item.lifetime>0)item._timeout=setTimeout(()=>{this.close(item);},item.lifetime);},close(item){if(item._timeout)clearTimeout(item._timeout);this.items.splice(this.items.indexOf(item),1);},info(msg){this.show(msg,{type:'primary',icon:'info-circle'});},success(msg){this.show(msg,{type:'success',icon:'check-circle'});},error(msg){this.show(msg,{type:'error',icon:'ban'});},warning(msg){this.show(msg,{type:'warning',icon:'exclamation-circle'});},alert(msg,title){this.show(msg,{x:'center',y:'top',title:title||location.host,lifetime:0,buttons:{OK:()=>{}}});},}});jQuery.fn.elapsedTime=function(selector,source,options={}){var options=jQuery.extend({lang:{years:['год','года','лет'],months:['месяц','месяца','месяцев'],days:['день','дня','дней'],hours:['час','часа','часов'],minutes:['минута','минуты','минут'],seconds:['секунда','секунды','секунд'],end:" назад",freshly:"только что",},plurar:function(n){return(n%10==1&&n%100!=11?0:n%10>=2&&n%10<=4&&(n%100<10||n%100>=20)?1:2);},intervalUpdate:1,},options);options.intervalUpdate*=1e3;var timeDifference=function(end,begin){if(end<begin){return false;}
...
return result.reverse().join(' ');};var elapsedTime=function(){var need_to_time_update=$(selector);if(need_to_time_update.size()>0){need_to_time_update.each(function(i){var date=need_to_time_update.eq(i).attr(source).toString().split(",");if(!date[5]){date[5]=0;}
...
else{need_to_time_update.eq(i).html(options.lang.freshly);}});}};elapsedTime();setInterval(elapsedTime,options.intervalUpdate);};$("body").elapsedTime('.elapsedTime','data-elapse_time',{lang:{years:[l('elt-years_0'),l('elt-years_1'),l('elt-years_2')],months:[l('elt-months_0'),l('elt-months_1'),l('elt-months_2')],days:[l('elt-days_0'),l('elt-days_1'),l('elt-days_2'),],hours:[l('elt-hours_0'),l('elt-hours_1'),l('elt-hours_2'),],minutes:[l('elt-minutes_0'),l('elt-minutes_1'),l('elt-minutes_2'),],seconds:[l('elt-seconds_0'),l('elt-seconds_1'),l('elt-seconds_2'),],end:l('elt-end'),freshly:l('elt-freshly'),}});jQuery.fn.countdown=function(selector,source,options={}){var options=jQuery.extend({lang:{years:['год','года','лет'],months:['месяц','месяца','месяцев'],days:['день','дня','дней'],hours:['час','часа','часов'],minutes:['минуту','минуты','минут'],seconds:['секунду','секунды','секунд'],after:" назад",deadline:"только что",},plurar:function(n){return(n%10==1&&n%100!=11?0:n%10>=2&&n%10<=4&&(n%100<10||n%100>=20)?1:2);},intervalUpdate:1,},options);options.intervalUpdate*=1e3;var timeDifference=function(end,begin){if(end>begin){return false;}
var difference={seconds:[begin.getSeconds()-end.getSeconds(),60],minutes:[begin.getMinutes()-end.getMinutes(),60],hours:[begin.getHours()-end.getHours(),24],days:[begin.getDate()-end.getDate(),new Date(begin.getYear(),begin.getMonth()+1,0).getDate()],months:[begin.getMonth()-end.getMonth(),12],years:[begin.getYear()-end.getYear(),0]};if(difference.years[0]!=0){delete(difference.minutes);delete(difference.seconds);}
else if(difference.months[0]!=0){delete(difference.minutes);delete(difference.seconds);}
else if(difference.days[0]!=0){delete(difference.seconds);}
else if(difference.hours[0]!=0){delete(difference.seconds);}
...
if(!difference[i][0]){continue;}
result.push(difference[i][0]+' '+options.lang[i][options.plurar(difference[i][0])]);}
return result.reverse().join(' ');};var countdown=function(){var need_to_time_update=$(selector);if(need_to_time_update.size()>0){need_to_time_update.each(function(i){var date=need_to_time_update.eq(i).attr(source).toString().split(",");if(!date[5]){date[5]=0;}
var s=timeDifference(new Date(),new Date(date[0],date[1]-1,date[2],date[3],date[4],date[5]));if(s.length){need_to_time_update.eq(i).html(options.lang.after+s);}
else{need_to_time_update.eq(i).html(options.lang.deadline);}});}};countdown();setInterval(countdown,options.intervalUpdate);};$("body").countdown('.countdown','data-countdown',{lang:{years:[l('elt-years_0'),l('elt-years_1'),l('elt-years_2')],months:[l('elt-months_0'),l('elt-months_1'),l('elt-months_2')],days:[l('elt-days_0'),l('elt-days_1'),l('elt-days_2'),],hours:[l('elt-hours_0'),l('elt-hours_1'),l('elt-hours_2'),],minutes:[l('elt-minutes_3'),l('elt-minutes_1'),l('elt-minutes_2'),],seconds:[l('elt-seconds_3'),l('elt-seconds_1'),l('elt-seconds_2'),],after:l('elt-after'),deadline:l('elt-deadline'),}});})();function $_GET(params,read){let query={};if(typeof params=='object'){query=params}
let urlParams=new URLSearchParams(window.location.search);urlParams.forEach((val,key)=>{query[key]=val;});if(read){let i=0;let str='?';for(key in query){str+=i==0?`${key}=${encodeURIComponent(query[key])}`:`&${key}=${encodeURIComponent(query[key])}`;i++;}
return str;}else{if(typeof params=='object'){return query;}else if(typeof params=='string'){return query.hasOwnProperty(params)?query[params]:null;}else{return query;}}}